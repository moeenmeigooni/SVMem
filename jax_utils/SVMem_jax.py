...
#                 b = b.at[k].set(j)
#                 k += 1
            (a, b, k) = jax.lax.cond((i < j), 
                                     lambda a, b, k, i, j: (a.at[k].set(i), b.at[k].set(j), k+1), 
                                     lambda a, b, k, i, j: (a, b, k), 
...
#                     dr -= box_dims[ri]
            def while_func(dr):
                ...
                return dr
            dr = jax.lax.cond(pred=periodic[ri].item(), true_fun=while_func, false_fun=lambda dr: dr, operand=dr)
            dist_mat = dist_mat.at[k].add(jnp.square(dr))
